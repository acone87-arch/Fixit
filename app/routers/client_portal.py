...
async def grant_access(user_id: uuid.UUID, client_id: uuid.UUID, site_id: uuid.UUID | None = None,
                       db: AsyncSession = Depends(get_db), user: CurrentUser = Depends(require_roles(UserRole.admin, UserRole.dispatcher))):
    member = await db.scalar(select(OrganizationMembership).where(
        OrganizationMembership.organization_id == user.organization_id, OrganizationMembership.user_id == user_id,
        OrganizationMembership.is_active.is_(True), OrganizationMembership.role.in_({UserRole.client_admin, UserRole.client_site_user}),
    ))
    ...
    if site_id:
        site = await db.scalar(select(Site).where(Site.id == site_id, Site.client_id == client_id, Site.organization_id == user.organization_id))
        if not site: raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Объект не принадлежит клиенту")
    access = ClientUserAccess(organization_id=user.organization_id, user_id=user_id, client_id=client_id, site_id=site_id)
    db.add(access)
    try: await db.commit()
    except Exception as exc:
        await db.rollback(); raise HTTPException(status.HTTP_409_CONFLICT, "Такой доступ уже назначен") from exc
    return {"id": str(access.id), "client_id": str(client_id), "site_id": str(site_id) if site_id else None}
