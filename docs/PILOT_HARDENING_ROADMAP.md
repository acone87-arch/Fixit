...
**Статус: 🟢 выполнено и проверено.** CI [34744686841](https://github.com/acone87-arch/Fixit/actions/runs/34744686841), код `386d09a6d0c703a399c74fed0e2396a0c414b542`: 293 passed, 0 failed, 0 skipped; 5 JS runtime-файлов и 16 Chromium IDB/SW cases.

**Definition of Done:** администратор создаёт Site и партию пустых Equipment, повторно скачивает печатный PDF с постоянными QR; менеджер назначенного объекта или назначенный клиенту техник сканирует QR на телефоне, заполняет карточку и переходит к следующей. После заполнения сохраняются исходные Equipment ID и public QR, а сущность участвует в обычной цепочке Equipment → QR → ServiceRequest → Repair → Act → Equipment History. Администратор редактирует все рабочие поля заполненной карточки.

| ID | Статус | Задача и доказательство |
|---|---|---|
| INV-01 | 🟢 выполнено и проверено | Атомарная партия 1–500 Equipment, постоянный idempotency key, повтор без дубликатов, повторное скачивание PDF и счётчик заполнения |
| INV-02 | 🟢 выполнено и проверено | A4 PDF: 8 этикеток 90×60 мм на листе, QR 40 мм, номер/партия/Site; ссылки проверены программно, две страницы отрендерены и просмотрены |
| INV-03 | 🟢 выполнено и проверено | Пустая карточка имеет явный inventory_pending; тип и серийный номер не выдумываются; заявка, QR intake и offline repair заблокированы до заполнения |
| INV-04 | 🟢 выполнено и проверено | Owner/admin, менеджер назначенного объекта и назначенный клиенту техник заполняют карточку; чужой Client/Site/tenant получает отказ |
| INV-05 | 🟢 выполнено и проверено | Реальный мобильный Chromium: сканирование, заполнение, фото, потеря ответа после commit, безопасный повтор и кнопка следующего QR |
| INV-06 | 🟢 выполнено и проверено | Полное редактирование owner/admin: тип (включая новый), производитель, модель, серийный номер, расположение, Site, статус и фото; optimistic version и уникальный serial защищают конкуренцию |
| INV-07 | 🟢 выполнено и проверено | PostgreSQL подтверждает сохранение ID/QR и полный путь до ServiceRequest, Repair, PDF-акта и Equipment History |
| INV-08 | 🟢 выполнено и проверено | Миграция 0015 сохраняет старые Equipment/QR и отмечает их заполненными; downgrade после печати партии запрещён, чтобы не потерять постоянные ссылки |

**Код:** `app/routers/inventory.py`, `app/services/inventory_pdf.py`, `app/static/inventory.js`, Equipment API/UI, миграция `20260910_0015`.

**Ограничения выпуска:** изменение проверено в draft PR №8, но не развёрнуто. Перед production требуется отдельная разрешённая выкладка миграции 0015 с backup и exact SHA; rollback приложения должен сохранять новую схему. Реальная печать на конкретной модели принтера и физическое сканирование Android/iOS остаются эксплуатационной проверкой.
...
- Исправления `d6a16e8b4cd676f6f667249f3ca9febc954d9175`, [Actions 34308457597](https://github.com/acone87-arch/Fixit/actions/runs/34308457597): **5 failed, 271 passed**, 661 предупреждение. Исходные фото, empty-result guard, legacy история и длинная диагностика уже прошли до следующих проверок. Выявлено: открытый passport modal перекрывает скачивание PDF при переходе к заявке; теперь закрывается до перехода. Новый обработчик скачивания результата использует доступный API/blob путь с обработкой ошибок.
- Три PDF failures — тест сравнивал строку без учёта переноса PDF-строк. Проверено генерацией настоящего акта и двумя парсерами: диагностика/работы/деталь присутствуют полностью. Сравнение теперь нормализует только пробельные символы и проверяет также всю длинную диагностику.
- Сквозной browser case дополнен waiting_parts → resume → client approval → reload → completion, с проверкой сохранённых диагностики/работ/детали/фото. Комментарий в форме явно обозначен как видимый клиенту.
...
### 10.09.2026 — P0.5: функциональная приёмка и подготовка выкладки

- Проверенный код `45b13c5007d480f5a22febe013265b9da3648520`, [Actions 34368558081](https://github.com/acone87-arch/Fixit/actions/runs/34368558081): **success**, **279 passed, 0 failed, 0 skipped**, 780 предупреждений, 230.50 s. Включены 103 PostgreSQL cases, 11 Chromium API/PG cases и 165 остальных Python tests. Пять JS runtime-файлов и отдельно **14 real Chromium IDB/SW cases** прошли.
- Подтверждены реальные commit/lost response для Repair и attachment, закрытие страницы и восстановление Blob/data_url; один Repair и одно attachment после retry; шесть concurrent upload возвращают один attachment.
- Подготовлена выкладка точно принятого SHA через `codex/pilot-release-p0-5`, без merge PR и изменения main. Ветка выкладки будет продвинута только после финального зелёного CI.
...
### 10.09.2026 — отдельный этап перед P0.6: QR-инвентаризация

- По запросу пользователя добавляется сценарий Site → количество → партия пустых Equipment и QR → PDF → сканирование на объекте → заполнение карточки → следующий QR. Администратор, менеджер назначенного объекта и назначенный клиенту техник могут заполнять пустую карточку; полное редактирование заполненной карточки доступно owner/admin. P0.6/P0.7 не начинались.
- База `05893a090389cba74409e10c56b9b2c2097c184f` (принятые P0.1–P0.5), ветка `codex/qr-inventory`. Main сверена: `87e3044f30a40b6d0cff0306e71f9aee83aca329`, identical 0/0. Production остаётся на `90d9421`; новый этап не выложен.
- Equipment сохраняет постоянные id/public_qr_token. Пустая карточка явно помечена inventory_pending, тип и серийный номер не выдумываются; сервисная заявка/ремонт до заполнения запрещены. Партии имеют постоянный ключ повтора, 1–500 единиц, повторный PDF и счётчик заполнения. A4: 8 этикеток 90×60 мм, QR 40 мм, печать 100%.
- Полное редактирование: тип, производитель, модель, серийный номер, расположение, объект, статус и фото. Имя по-прежнему определяется типом оборудования. Проверка версии защищает от перезаписи чужих изменений. Старые API status/site и offline payloads сохранены.
- Миграция 0015 добавляет партии/состояние инвентаризации; прежние Equipment автоматически считаются заполненными, ID/QR не меняются. После появления партий downgrade запрещён: нельзя потерять напечатанные ссылки или подставлять фиктивные серийные номера. Выкладка потребует отдельной проверки совместимого rollback.
- Добавлены HTTP/PostgreSQL проверки партий/retry/ACL/конкуренции/полного сервисного пути, миграции старых Equipment, мобильного заполнения после сканирования и потери ответа, администраторского редактирования. Итоговая приёмка выполняется; готовность пока не объявляется.

### 11.09.2026 — QR-инвентаризация: результаты первой приёмки

- [CI 34495963575](https://github.com/acone87-arch/Fixit/actions/runs/34495963575), код `0754072`: **292 passed, 1 failed, 0 skipped**. Все 10 новых сценариев прошли, включая реальные PostgreSQL, миграцию и два Chromium E2E. Ошибка старого unit double: SimpleNamespace Equipment не имел inventory_pending; double обновлён как обычная заполненная запись. Это не ошибка реального ORM пути и не основание пропускать старый тест. Точечный повтор: 13 passed.
- Локальная Python-проверка до обновления double: 165 passed, 1 failed, 127 skipped (PostgreSQL/полный browser/Linux недоступны локально). После обновления runtime создано отдельное .venv-inventory; зависание pip metadata устранено отключением кэша для дочерних build процессов.
- PDF-проба на 9 QR отрендерена Poppler; обе страницы просмотрены. Кириллица, QR 40 мм, 8 этикеток на первой странице, девятая на второй, границы и номера читаются. Это синтетический QA-образец, не партия production.
- Мобильный E2E дополнен загрузкой фото и проверкой ширины экрана; редактирование — созданием нового типа. Owner получает ту же кнопку удаления фото, что admin, в соответствии с существующим серверным guard. Итоговый CI обновлённого кода ожидается.

### 13.09.2026 — QR-инвентаризация: итоговая приёмка

- [CI 34744686841](https://github.com/acone87-arch/Fixit/actions/runs/34744686841), код `386d09a6d0c703a399c74fed0e2396a0c414b542`: **293 passed, 0 failed, 0 skipped**, 896 предупреждений зависимостей, 262.84 s. В составе 111 PostgreSQL, 13 Chromium/API и 169 остальных Python cases; дополнительно прошли все 5 JS runtime-файлов и 16 настоящих Chromium IDB/SW cases.
- Новые 10 cases: 7 API/PostgreSQL сценариев партий, ролей, idempotency, конкуренции и полной сервисной цепочки; 1 PostgreSQL migration case; 2 мобильных Chromium E2E. Первый CI `34495963575` дал 292 passed и 1 failed только из-за старого SimpleNamespace без нового поля; после корректировки regression сохранён и финальный suite зелёный.
