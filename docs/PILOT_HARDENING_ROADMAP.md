...
**Definition of Done:** администратор создаёт Site и партию пустых Equipment, повторно скачивает печатный PDF с постоянными QR; назначенный клиенту техник сканирует QR на телефоне, заполняет карточку и переходит к следующей. После заполнения сохраняются исходные Equipment ID и public QR, а сущность участвует в обычной цепочке Equipment → QR → ServiceRequest → Repair → Act → Equipment History. Администратор редактирует все рабочие поля заполненной карточки.
...
| INV-04 | 🟢 выполнено и проверено | Owner/admin и назначенный клиенту техник заполняют карточку; чужой Client/Site/tenant и клиентские роли получают отказ |
